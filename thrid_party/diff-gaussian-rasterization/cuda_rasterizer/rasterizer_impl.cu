...
void CudaRasterizer::Rasterizer::backward(
    ...
    char *img_buffer, const float *dL_dpix, const float *dL_depths,
    float *dL_dmean2D, float *dL_dconic, float *dL_dopacity, float *dL_dcolor,
  ...
  CHECK_CUDA(BACKWARD::render(tile_grid, block, imgState.ranges,
                              binningState.point_list, width, height,
                              background, geomState.means2D,
                              geomState.conic_opacity, color_ptr, depth_ptr,
                              imgState.accum_alpha, imgState.n_contrib, dL_dpix,
                              dL_depths, (float3 *)dL_dmean2D,

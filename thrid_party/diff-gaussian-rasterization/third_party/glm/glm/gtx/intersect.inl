...
intersectRayPlane(genType const &orig, genType const &dir,
  ...
  if (glm::abs(d) > Epsilon) // if dir and planeNormal are not perpendicular
  {
    ...
    if (tmp_intersectionDistance > static_cast<typename genType::value_type>(
                                       0)) { // allow only intersections
...
intersectRayTriangle(vec<3, T, Q> const &orig, vec<3, T, Q> const &dir,
                     vec<3, T, Q> const &vert0, vec<3, T, Q> const &vert1,
                     vec<3, T, Q> const &vert2, vec<2, T, Q> &baryPosition,
                     T &distance) {

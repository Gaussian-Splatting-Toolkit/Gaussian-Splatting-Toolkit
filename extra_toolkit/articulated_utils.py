...
def rotate_and_combine(
    angle, total_angle, axis_of_rotation, cuff_point_cloud, rest_point_cloud
):
    ...
    center_of_rotation = np.array(
        [(a + b) / 2 for a, b in zip(end_1, end_2)]
    )  # Replace x, y, z with the coordinates of the connection point

    R = cuff_point_cloud.get_rotation_matrix_from_axis_angle(
        axis_of_rotation * np.radians(angle)
    )
    ...
    o3d.io.write_point_cloud(
        base_path + f"interim_rotate_angle_{total_angle}.ply", combined_point_cloud
    )


if __name__ == "__main__":
    base_path = (
        "/mnt/d/Projects/reconstruction/result/small_obj/orange_clothes/move/stage_3/"
    )

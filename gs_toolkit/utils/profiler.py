...
def time_function(
    name_or_func: Union[CallableT, str]
) -> Union[CallableT, ContextManager[Any]]:
...
class _TimeFunction(ContextDecorator):
    ...
    def __new__(cls, func: Union[str, Callable]):
        ...
        raise ValueError(
            f"Argument func of type {type(func)} is not a string or a callable."
        )
        ...
        if function.endswith("train_iteration") or function.endswith("eval_iteration"):
            ...
            if step in self.trace_steps:
                ...
                prof.export_chrome_trace(
                    str(
                        self.output_path
                        / f"trace_{stage}_{step}{'_blocking' if launch_kernel_blocking else ''}.json"
                    )
                )
                return

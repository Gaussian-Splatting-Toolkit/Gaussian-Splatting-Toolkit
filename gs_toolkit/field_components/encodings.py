...
class PolyhedronFFEncoding(FFEncoding):
    ...
    def __init__(
        ...
        super().__init__(
            3, basis_t, num_frequencies, min_freq_exp, max_freq_exp, include_input
        )
...
class HashEncoding(Encoding):
    ...
    def __init__(
        ...
        self.growth_factor = (
            np.exp((np.log(max_res) - np.log(min_res)) / (num_levels - 1))
            if num_levels > 1
            else 1
        )
    ...
    def hash_fn(
        self, in_tensor: Int[Tensor, "*bs num_levels 3"]
...
class TensorCPEncoding(Encoding):
    ...
    def __init__(
        self, resolution: int = 256, num_components: int = 24, init_scale: float = 0.1
    ) -> None:
        super().__init__(in_dim=3)
...
class TensorVMEncoding(Encoding):
    ...
    def __init__(
        ...
        self.plane_coef = nn.Parameter(
            init_scale * torch.randn((3, num_components, resolution, resolution))
        )
        self.line_coef = nn.Parameter(
            init_scale * torch.randn((3, num_components, resolution, 1))
        )
    ...
    def forward(
        ...
        features = torch.moveaxis(
            features.view(3 * self.num_components, *in_tensor.shape[:-1]), 0, -1
        )
...
class TriplaneEncoding(Encoding):
    ...
    def forward(
        self, in_tensor: Float[Tensor, "*bs 3"]
    ) -> Float[Tensor, "*bs num_components featuresize"]:
    ...
    def __init__(
        ...
        for coo_comb in self.coo_combs:
            new_plane_coef = nn.Parameter(
                torch.empty(
                    [self.num_components]
                    + [self.resolution[cc] for cc in coo_comb[::-1]]
                )
            )

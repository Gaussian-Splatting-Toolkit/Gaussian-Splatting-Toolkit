"""Helper utils for processing polycam data into the gs_toolkit format."""
...
from gs_toolkit.utils import io
...
def polycam_to_json(
    ...
    """Convert Polycam data into a gs_toolkit dataset.
    ...
    for i, image_filename in enumerate(image_filenames):
        ...
        if use_depth:
            frame[
                "depth_file_path"
            ] = f"./depth/frame_{i+1:05d}{depth_filenames[i].suffix}"
...
def process_depth_maps(
    ...
    (
        ...
        num_orig_depth_maps,
    ) = process_data_utils.get_image_filenames(polycam_depth_dir, max_dataset_size)

    # Copy depth images to output directory
    copied_depth_maps_paths = (
        process_data_utils.copy_and_upscale_polycam_depth_maps_list(
            polycam_depth_maps_filenames,
            depth_dir=depth_dir,
            num_downscales=num_downscales,
            crop_border_pixels=crop_border_pixels,

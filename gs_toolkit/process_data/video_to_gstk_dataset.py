"""Processes a video to a gs_toolkit compatible dataset."""
...
@dataclass
class VideoToGSToolkitDataset(ColmapConverterToGSToolkitDataset):
    """Process videos into a gs_toolkit dataset.
    ...
    def main(self) -> None:
        """Process video into a gs_toolkit dataset."""
        ...
        if self.camera_type == "equirectangular":
            ...
            summary_log.append(
                process_data_utils.downscale_images(
                    self.image_dir, self.num_downscales, verbose=self.verbose
                )
            )

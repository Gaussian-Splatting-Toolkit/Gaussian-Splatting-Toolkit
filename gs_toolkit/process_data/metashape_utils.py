"""Helper utils for processing metashape data into the gs_toolkit format."""
...
def metashape_to_json(
    ...
    """Convert Metashape data into a gs_toolkit dataset.
    ...
    if sensor_type.count(sensor_type[0]) != len(sensor_type):
        raise ValueError(
            ...
            "gs_toolkit does not support per-frame camera_model types."
    ...
    for sensor in calibrated_sensors:
        ...
        if calib is None:
            assert (
                sensor_type[0] == "spherical"
            ), "Only spherical sensors should have no intrinsics"
            s["fl_x"] = s["w"] / 2.0
            s["fl_y"] = s["h"]
            s["cx"] = s["w"] / 2.0
            s["cy"] = s["h"] / 2.0
        ...
        for component in components:
            ...
            if transform is not None:
                ...
                if rotation is None:
                    ...
                    r = np.array([float(x) for x in rotation.text.split()]).reshape(
                        (3, 3)
                    )
    ...
    for camera in cameras:
        ...
        if sensor_id not in sensor_dict:
            ...
            if verbose:
                CONSOLE.print(
                    f"Missing sensor calibration for {camera.get('label')}, Skipping"
                )
            num_skipped += 1
            continue

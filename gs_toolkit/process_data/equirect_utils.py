...
def remap_cubic(
    ...
    return torch.nn.functional.grid_sample(
        img, grid, mode="bicubic", padding_mode="zeros"
    )


def equirect2persp(
    img: torch.Tensor, fov: int, theta: int, phi: int, hd: int, wd: int
) -> torch.Tensor:
    ...
    z_map = -torch.tile(
        (torch.arange(0, hd, device=device) - c_y) * h_interval, [wd, 1]
    ).T
    ...
    R2 = euler_rodriguez_rotation_matrix(
        torch.mm(R1, y_axis.view(3, 1)).squeeze(), torch.deg2rad(-phi_tensor)
    ).to(device)
...
def generate_planar_projections_from_equirectangular(
    ...
    for i in crop_factor:
        if i < 0 or i > 1:
            CONSOLE.print(
                "[bold red] Invalid crop factor. All values must be in [0,1]."
            )
            sys.exit(1)

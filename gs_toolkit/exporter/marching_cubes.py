...
def create_point_pyramid(
    points: Float[Tensor, "3 height width depth"]
) -> List[Float[Tensor, "3 height width depth"]]:
    """
...
def evaluate_sdf(
    sdf: Callable[[Tensor], Tensor], points: Float[Tensor, "batch 3"]
) -> Float[Tensor, "batch"]:
    """
    ...
    for i in range(N):
        for j in range(N):
            for k in range(N):
                ...
                pts_sdf = evaluate_multiresolution_sdf(
                    evaluate, points_pyramid, coarse_mask, x_max, x_min, crop_n
                )
                ...
                if current_mask is not None:
                    ...
                    if valid_z.shape[0] <= 0 or (
                        np.min(valid_z) > isosurface_threshold
                        or np.max(valid_z) < isosurface_threshold
                    ):
                        continue

                if not (
                    np.min(z) > isosurface_threshold or np.max(z) < isosurface_threshold
                ):

...
__device__ __constant__ float SH_C4[] = {
    2.5033429417967046f,  -1.7701307697799304,  0.9461746957575601f,
...
__device__ void sh_coeffs_to_color(const unsigned degree, const float3 &viewdir,
                                   const float *coeffs, float *colors) {
  ...
  float norm = sqrt(viewdir.x * viewdir.x + viewdir.y * viewdir.y +
                    viewdir.z * viewdir.z);
  ...
  for (int c = 0; c < CHANNELS; ++c) {
    colors[c] +=
        SH_C1 * (-y * coeffs[1 * CHANNELS + c] + z * coeffs[2 * CHANNELS + c] -
                 x * coeffs[3 * CHANNELS + c]);
    if (degree < 2) {
      continue;
    }
    colors[c] += (SH_C2[0] * xy * coeffs[4 * CHANNELS + c] +
...
__device__ void sh_coeffs_to_color_vjp(const unsigned degree,
                                       const float3 &viewdir,
                                       const float *v_colors, float *v_coeffs) {
// Expects v_colors to be len CHANNELS
// and v_coeffs to be num_bases * CHANNELS
#pragma unroll
  for (int c = 0; c < CHANNELS; ++c) {
...
__global__ void compute_sh_forward_kernel(const unsigned num_points,
  ...
  sh_coeffs_to_color(degrees_to_use, viewdirs[idx], &(coeffs[idx_sh]),
                     &(colors[idx_col]));
}

__global__ void compute_sh_backward_kernel(const unsigned num_points,
                                           const unsigned degree,
                                           const unsigned degrees_to_use,
                                           const float3 *__restrict__ viewdirs,
  ...
  sh_coeffs_to_color_vjp(degrees_to_use, viewdirs[idx], &(v_colors[idx_col]),
                         &(v_coeffs[idx_sh]));
}

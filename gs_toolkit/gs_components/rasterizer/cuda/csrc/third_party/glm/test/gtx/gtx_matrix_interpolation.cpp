...
template <class T>
int testForAxisAngle(glm::vec<3, T, glm::defaultp> const axisTrue,
                     T const angleTrue) {
  T const eps = std::sqrt(std::numeric_limits<T>::epsilon());

  glm::mat<4, 4, T, glm::defaultp> const matTrue =
      glm::axisAngleMatrix(axisTrue, angleTrue);

  glm::vec<3, T, glm::defaultp> axis;
  T angle;
  glm::axisAngle(matTrue, axis, angle);
  glm::mat<4, 4, T, glm::defaultp> const matRebuilt =
      glm::axisAngleMatrix(axis, angle);

  glm::mat<4, 4, T, glm::defaultp> const errMat = matTrue - matRebuilt;
  T const maxErr = glm::compMax(glm::vec<4, T, glm::defaultp>(
      glm::compMax(glm::abs(errMat[0])), glm::compMax(glm::abs(errMat[1])),
      glm::compMax(glm::abs(errMat[2])), glm::compMax(glm::abs(errMat[3]))));
...
static int test_axisAngle2() {
  ...
  Error +=
      testForAxisAngle(glm::vec3(0.358f, 0.0716f, 0.9309f), -glm::pi<float>());
  Error +=
      testForAxisAngle(glm::vec3(1.0f, 0.0f, 0.0f), glm::pi<float>() + 2e-6f);
  Error +=
      testForAxisAngle(glm::vec3(1.0f, 0.0f, 0.0f), glm::pi<float>() + 1e-4f);
  Error +=
      testForAxisAngle(glm::vec3(0.0f, 1.0f, 0.0f), -glm::pi<float>() + 1e-3f);
  Error += testForAxisAngle(glm::vec3(0.358f, 0.0716f, 0.9309f),
                            -glm::pi<float>() + 5e-3f);

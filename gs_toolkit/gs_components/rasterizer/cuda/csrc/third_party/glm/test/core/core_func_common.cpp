...
int min_tern(int a, int b) { return a < b ? a : b; }

int min_int(int x, int y) { return y ^ ((x ^ y) & -(x < y)); }
...
static int test() {
...

  {
    ...
    Error += (glm::equal(B, 1.0f, glm::epsilon<float>()) ||
              glm::equal(B, 0.0f, glm::epsilon<float>()))
                 ? 0
                 : 1;

...
GLM_FUNC_QUALIFIER qua<T, Q> log(qua<T, Q> const &q) {
  ...
  if (Vec3Len < epsilon<T>()) {
    if (q.w > static_cast<T>(0))
      return qua<T, Q>::wxyz(log(q.w), static_cast<T>(0), static_cast<T>(0),
                             static_cast<T>(0));
    else if (q.w < static_cast<T>(0))
      return qua<T, Q>::wxyz(log(-q.w), pi<T>(), static_cast<T>(0),
                             static_cast<T>(0));
    else
    ...
    return qua<T, Q>::wxyz(static_cast<T>(0.5) * log(QuatLen2), t * q.x,
                           t * q.y, t * q.z);
  }
}
...
GLM_FUNC_QUALIFIER qua<T, Q> pow(qua<T, Q> const &x, T y) {
  ...
  return qua<T, Q>::wxyz(cos(NewAngle) * magnitude * Mag, x.x * Div * Mag,

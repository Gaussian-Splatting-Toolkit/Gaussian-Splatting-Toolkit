...
__global__ void project_gaussians_forward_kernel(
    ...
    const dim3 img_size, const dim3 tile_bounds, const float clip_thresh,
    float *__restrict__ covs3d, float2 *__restrict__ xys,
    float *__restrict__ depths, int *__restrict__ radii,
    float3 *__restrict__ conics, int32_t *__restrict__ num_tiles_hit) {
...
__global__ void map_gaussian_to_intersects(
    ...
    int64_t *__restrict__ isect_ids, int32_t *__restrict__ gaussian_ids) {
...
__device__ float3 project_cov3d_ewa(const float3 &__restrict__ mean3d,
  ...
  glm::mat3 J = glm::mat3(fx * rz, 0.f, 0.f, 0.f, fy * rz, 0.f, -fx * t.x * rz2,
                          -fy * t.y * rz2, 0.f);
  glm::mat3 T = J * W;
  ...
  return make_float3(float(cov[0][0]) + 0.3f, float(cov[0][1]),
                     float(cov[1][1]) + 0.3f);
}

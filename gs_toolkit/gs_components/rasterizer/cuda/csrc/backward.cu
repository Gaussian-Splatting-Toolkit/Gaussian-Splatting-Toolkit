...
__global__ void nd_rasterize_backward_kernel(
    ...
    const float *__restrict__ v_output, float2 *__restrict__ v_xy,
    float3 *__restrict__ v_conic, float *__restrict__ v_rgb,
    float *__restrict__ v_opacity, float *__restrict__ workspace) {
  ...
  for (int idx = bin_final - 1; idx >= range.x; --idx) {
    ...
    atomicAdd(&(v_xy[g].x), v_sigma * (conic.x * delta.x + conic.y * delta.y));
    atomicAdd(&(v_xy[g].y), v_sigma * (conic.y * delta.x + conic.z * delta.y));
  }
}
...
__global__ void rasterize_backward_kernel(
    ...
    const float *__restrict__ final_Ts, const int *__restrict__ final_index,
    const float3 *__restrict__ v_output, float2 *__restrict__ v_xy,
    float3 *__restrict__ v_conic, float3 *__restrict__ v_rgb,
...
__global__ void project_gaussians_backward_kernel(
    ...
    const float *__restrict__ projmat, const float4 intrins,
    const dim3 img_size, const float *__restrict__ cov3d,
    const int *__restrict__ radii, const float3 *__restrict__ conics,
    const float2 *__restrict__ v_xy, const float *__restrict__ v_depth,
    const float3 *__restrict__ v_conic, float3 *__restrict__ v_cov2d,
...
__device__ void scale_rot_to_cov3d_vjp(const float3 scale,
  ...
  glm::mat3 v_V = glm::mat3(v_cov3d[0], 0.5 * v_cov3d[1], 0.5 * v_cov3d[2],

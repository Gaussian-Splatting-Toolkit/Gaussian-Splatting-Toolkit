...
project_gaussians_backward_tensor(
    ...
    torch::Tensor &compensation, torch::Tensor &v_xy, torch::Tensor &v_depth,
    torch::Tensor &v_conic, torch::Tensor &v_compensation) {
  DEVICE_GUARD(means3d);
  dim3 img_size_dim3;
  img_size_dim3.x = img_width;
  img_size_dim3.y = img_height;
...
nd_rasterize_forward_tensor(const std::tuple<int, int, int> tile_bounds,
  ...
  const uint32_t shared_mem = B * sizeof(int) + B * sizeof(float3) +
                              B * sizeof(float3) + B * channels * sizeof(half);
  if (cudaFuncSetAttribute(nd_rasterize_forward,
                           cudaFuncAttributeMaxDynamicSharedMemorySize,
                           shared_mem) != cudaSuccess) {
    AT_ERROR("Failed to set maximum shared memory size (requested ", shared_mem,
             " bytes), try lowering block_size");
  }
...
nd_rasterize_backward_tensor(
  ...
  // shared mem accounts for each thread having a local shared memory workspace
  // for running sum
  const uint32_t shared_mem = B * channels * sizeof(half);
  if (cudaFuncSetAttribute(nd_rasterize_backward_kernel,
                           cudaFuncAttributeMaxDynamicSharedMemorySize,
                           shared_mem) != cudaSuccess) {
    AT_ERROR("Failed to set maximum shared memory size (requested ", shared_mem,
             " bytes), try lowering block_size");

"""
Custom collate function that includes cases for gs_toolkit types.
"""
...
GS_TOOLKIT_COLLATE_ERR_MSG_FORMAT = (
...
def gs_toolkit_collate(
    batch: Any, extra_mappings: Union[Dict[type, Callable], None] = None
) -> Any:
    r"""
    This is the default pytorch collate function, but with support for gs_toolkit types. All documentation
    below is copied straight over from pytorch's default_collate function, python version 3.8.13,
    pytorch version '1.12.1+cu113'. Custom gs_toolkit types are accounted for at the end, and extra
    ...
    from default_collate to gs_toolkit_collate, and the addition of the gs_toolkit_collate_err_msg_format variable.
    ...
    Here is the general input type (based on the type of the element within the batch) to output type mapping:
        ...
        * `Mapping[K, V_i]` -> `Mapping[K, gs_toolkit_collate([V_1, V_2, ...])]`
        * `NamedTuple[V1_i, V2_i, ...]` -> `NamedTuple[gs_toolkit_collate([V1_1, V1_2, ...]),
          gs_toolkit_collate([V2_1, V2_2, ...]), ...]`
        * `Sequence[V1_i, V2_i, ...]` -> `Sequence[gs_toolkit_collate([V1_1, V1_2, ...]),
          gs_toolkit_collate([V2_1, V2_2, ...]), ...]`
    ...
    Examples:
        ...
        >>> gs_toolkit_collate([0, 1, 2, 3])
        tensor([0, 1, 2, 3])
        >>> # Example with a batch of `str`s:
        >>> gs_toolkit_collate(['a', 'b', 'c'])
        ['a', 'b', 'c']
        >>> # Example with `Map` inside the batch:
        >>> gs_toolkit_collate([{'A': 0, 'B': 1}, {'A': 100, 'B': 100}])
        {'A': tensor([  0, 100]), 'B': tensor([  1, 100])}
        >>> # Example with `NamedTuple` inside the batch:
        >>> Point = namedtuple('Point', ['x', 'y'])
        >>> gs_toolkit_collate([Point(0, 0), Point(1, 1)])
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if elem_type.__name__ in ("ndarray", "memmap"):
            ...
            if np_str_obj_array_pattern.search(elem.dtype.str) is not None:
                raise TypeError(GS_TOOLKIT_COLLATE_ERR_MSG_FORMAT.format(elem.dtype))

            return gs_toolkit_collate(
        ...
        try:
            return elem_type(
                {
                    key: gs_toolkit_collate(
                        [d[key] for d in batch], extra_mappings=extra_mappings
                    )
                    for key in elem
                }
            )
        ...
        return elem_type(
            *(
                gs_toolkit_collate(samples, extra_mappings=extra_mappings)
    ...
    # GS Toolkit types supported below
        ...
        metadata = {
            key: op([cam.metadata[key] for cam in batch], dim=0)
        ...
        return Cameras(
            ...
            times=torch.stack(
                [
                    ...
                    else -torch.ones_like(cameras.times)
                    for cameras in batch
                ],
                dim=0,
            ),
    ...
    raise TypeError(GS_TOOLKIT_COLLATE_ERR_MSG_FORMAT.format(elem_type))

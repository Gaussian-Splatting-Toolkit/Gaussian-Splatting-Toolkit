...
class VanillaDataManagerConfig(DataManagerConfig):
    ...
    dataparser: AnnotatedDataParserUnion = field(
        default_factory=lambda: BlenderDataParserConfig()
    )
    ...
    pixel_sampler: PixelSamplerConfig = field(
        default_factory=lambda: PixelSamplerConfig()
    )
...
class VanillaDataManager(DataManager, Generic[TDataset]):
    ...
    def _get_pixel_sampler(
        ...
        if (
            dataset.cameras.metadata is not None
            and "fisheye_crop_radius" in dataset.cameras.metadata
        ):

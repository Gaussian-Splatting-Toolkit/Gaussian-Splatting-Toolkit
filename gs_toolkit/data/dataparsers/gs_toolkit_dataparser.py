...
from gs_toolkit.cameras.cameras import CAMERA_MODEL_TO_TYPE, Cameras, CameraType
...
class GSToolkit(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if "applied_scale" in meta:
            self.config.scale_factor = meta["applied_scale"]
        distort_fixed = False
        ...
        if "ply_file_path" in meta:
            ...
            metadata.update(
                self._load_3D_points(ply_file_path, transform_matrix, scale_factor)
            )

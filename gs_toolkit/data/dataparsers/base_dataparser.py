...
import gs_toolkit.configs.base_config as cfg
...
class DataparserOutputs:
    ...
    scene_box: SceneBox = field(
        default_factory=lambda: SceneBox(aabb=torch.tensor([[-1, -1, -1], [1, 1, 1]]))
    )
...
class DataParser:
    ...
    def _generate_dataparser_outputs(
        self, split: str = "train", **kwargs: Optional[Dict]
    ) -> DataparserOutputs:
...
def transform_poses_to_original_space(
    ...
    output_poses = torch.cat(
        (
            poses,
            torch.tensor(
                [[[0, 0, 0, 1]]], dtype=poses.dtype, device=poses.device
            ).repeat_interleave(len(poses), 0),
        ),
        1,
    )
    ...
    inv_transform = torch.linalg.inv(
        torch.cat(
            (
                ...
                torch.tensor(
                    [[0, 0, 0, 1]],
                    dtype=applied_transform.dtype,
                    device=applied_transform.device,
                ),
            ),
            0,
        )

...
class ViewerControl:
    ...
    def get_camera(
        self, img_height: int, img_width: int, client_id: Optional[int] = None
    ) -> Optional[Cameras]:
        """
...
class ViewerElement(Generic[TValue]):
    ...
    def __init__(
        ...
        self.gui_handle: Optional[
            Union[GuiInputHandle[TValue], GuiButtonHandle, GuiButtonGroupHandle]
        ] = None
...
class ViewerButton(ViewerElement[bool]):
    ...
    def __init__(
        self,
        name: str,
        cb_hook: Callable[[ViewerButton], Any],
        disabled: bool = False,
        visible: bool = True,
    ):
    ...
    def _create_gui_handle(self, viser_server: ViserServer) -> None:
        self.gui_handle = viser_server.add_gui_button(
            label=self.name, disabled=self.disabled, visible=self.visible
        )
...
class ViewerSlider(ViewerParameter[IntOrFloat], Generic[IntOrFloat]):
    ...
    def __init__(
        ...
        super().__init__(
            name, default_value, disabled=disabled, visible=visible, cb_hook=cb_hook
        )
        self.min = min_value
        self.max = max_value
        self.step = step
...
class ViewerButtonGroup(ViewerParameter[TString], Generic[TString]):
    ...
    def _create_gui_handle(self, viser_server: ViserServer) -> None:
        assert self.gui_handle is None, "gui_handle should be initialized once"
        self.gui_handle = viser_server.add_gui_button_group(
            self.name, self.options, visible=self.visible
        )

...
def populate_export_tab(
    server: viser.ViserServer, control_panel: ControlPanel, config_path: Path
) -> None:
...
def show_command_modal(
    client: viser.ClientHandle, what: Literal["mesh", "point cloud"], command: str
) -> None:
...
def populate_point_cloud_tab(
    ...
    generate_command = server.add_gui_button(
...
def populate_mesh_tab(
    ...
    output_directory = server.add_gui_text(

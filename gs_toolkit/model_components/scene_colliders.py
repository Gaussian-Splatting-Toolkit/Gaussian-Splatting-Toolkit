...
class AABBBoxCollider(SceneCollider):
    ...
    def _intersect_with_aabb(
        self,
        rays_o: Float[Tensor, "num_rays 3"],
        rays_d: Float[Tensor, "num_rays 3"],
...
class SphereCollider(SceneCollider):
    ...
    def __init__(
        self, center: torch.Tensor, radius: float, near_plane: float = 0.0, **kwargs
    ) -> None:
        super().__init__(**kwargs)
        self.center = center
        self.radius = radius
...
class NearFarCollider(SceneCollider):
    ...
    def set_nears_and_fars(self, ray_bundle: RayBundle) -> RayBundle:
        ...
        near_plane = (
            self.near_plane if (self.training or not self.reset_near_plane) else 0
        )

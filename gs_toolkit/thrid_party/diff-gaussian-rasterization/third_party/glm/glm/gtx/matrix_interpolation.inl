...
GLM_FUNC_QUALIFIER void axisAngle(mat<4, 4, T, Q> const &m, vec<3, T, Q> &axis,
                                  T &angle) {
  ...
  if (nearSymmetrical) {
    ...
    if (nearIdentity) {
      ...
      axis = vec<3, T, Q>(static_cast<T>(1.0), static_cast<T>(0.0),
                          static_cast<T>(0.0));
      return;
    }
    angle = pi<T>();
  ...
  T const angleCos =
      (m[0][0] + m[1][1] + m[2][2] - static_cast<T>(1)) * static_cast<T>(0.5);
  ...
  axis = glm::normalize(glm::vec<3, T, Q>(m[1][2] - m[2][1], m[2][0] - m[0][2],
                                          m[0][1] - m[1][0]));
}

template <typename T, qualifier Q>
GLM_FUNC_QUALIFIER mat<4, 4, T, Q> axisAngleMatrix(vec<3, T, Q> const &axis,
                                                   T const angle) {
  T c = cos(angle);
  T s = sin(angle);
  ...
  return mat<4, 4, T, Q>(
      ...
      static_cast<T>(0.0), t * n.x * n.y - n.z * s, t * n.y * n.y + c,
      t * n.y * n.z + n.x * s, static_cast<T>(0.0), t * n.x * n.z + n.y * s,
      t * n.y * n.z - n.x * s, t * n.z * n.z + c, static_cast<T>(0.0),
...
extractMatrixRotation(mat<4, 4, T, Q> const &m) {
  return mat<4, 4, T, Q>(m[0][0], m[0][1], m[0][2], static_cast<T>(0.0),

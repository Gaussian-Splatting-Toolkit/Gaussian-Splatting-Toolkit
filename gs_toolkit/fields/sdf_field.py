...
from gs_toolkit.field_components.embedding import Embedding
from gs_toolkit.field_components.encodings import NeRFEncoding
...
from gs_toolkit.fields.base_field import Field, FieldConfig
from gs_toolkit.utils.external import tcnn
...
class SDFField(Field):
    ...
    def __init__(
        ...
        self.embedding_appearance = Embedding(
            self.num_images, self.config.appearance_embedding_dim
        )
        ...
        growth_factor = np.exp(
            (np.log(config.max_res) - np.log(config.base_res)) / (config.num_levels - 1)
        )
        ...
        # color network
        dims = [
            self.config.hidden_dim_color for _ in range(self.config.num_layers_color)
        ]
        ...
        if self.use_grid_feature:
            assert (
                self.spatial_distortion is not None
            ), "spatial distortion must be provided when using grid feature"
    ...
    def forward_geonetwork(
        self, inputs: Float[Tensor, "*batch 3"]
        ...
        if self.use_grid_feature:
            ...
            feature = torch.zeros_like(
                inputs[:, :1].repeat(1, self.encoding.n_output_dims)
            )
    ...
    def get_sdf(self, ray_samples: RaySamples) -> Float[Tensor, "num_samples ... 1"]:
        ...
        hidden_output = self.forward_geonetwork(positions_flat).view(

...
class MessageApi(abc.ABC):
    ...
    def add_gui_checkbox(
        self, name: str, initial_value: bool, hint: Optional[str] = None
    ) -> GuiHandle[bool]:
    ...
    def add_gui_text(
        self, name: str, initial_value: str, hint: Optional[str] = None
    ) -> GuiHandle[str]:
    ...
    def add_gui_number(
        self, name: str, initial_value: IntOrFloat, hint: Optional[str] = None
    ...
    def set_background_image(
        ...
        media_type, base64_data = _encode_image_base64(
            image, file_format, quality=quality
        )
        self._queue(
            messages.BackgroundImageMessage(
                media_type=media_type, base64_data=base64_data
            )
        )

    def send_file_path_info(
        self, config_base_dir: Path, data_base_dir: Path, export_path_name: str
    ) -> None:
    ...
    def _add_gui_impl(
        ...
        if not is_button:
            ...
            def sync_other_clients(client_id: ClientId, value: Any) -> None:
                message = messages.GuiSetValueMessage(

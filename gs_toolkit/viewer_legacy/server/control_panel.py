...
from gs_toolkit.viewer_legacy.server.viewer_elements import (
    ViewerButtonGroup,
    ViewerCheckbox,
    ViewerDropdown,
    ViewerElement,
...
class ControlPanel:
    ...
    def __init__(
        ...
        self._invert = ViewerCheckbox(
            "Invert", False, cb_hook=rerender_cb, hint="Invert the colormap"
        )
        self._normalize = ViewerCheckbox(
            "Normalize", True, cb_hook=rerender_cb, hint="Normalize the colormap"
        )
        self._min = ViewerNumber(
            "Min", 0.0, cb_hook=rerender_cb, hint="Min value of the colormap"
        )
        self._max = ViewerNumber(
            "Max", 1.0, cb_hook=rerender_cb, hint="Max value of the colormap"
        )
        ...
        self._crop_min = ViewerVec3(
            ...
            0.05,
            cb_hook=crop_update_cb,
            hint="Minimum value of the crop",
        )
        self._crop_max = ViewerVec3(
            ...
            0.05,
            cb_hook=crop_update_cb,
            hint="Maximum value of the crop",
        )
        self._time = ViewerSlider(
            "Time", 0.0, 0.0, 1.0, 0.01, cb_hook=rerender_cb, hint="Time to render"
        )
        ...
        with self.viser_server.gui_folder("Split Screen"):
            ...
            with self.viser_server.gui_folder("  "):
                self.add_element(
                    self._split_invert, additional_tags=("split_colormap",)
                )
                self.add_element(
                    self._split_normalize, additional_tags=("split_colormap",)
                )

...
def get_viewer_url(websocket_port: int) -> str:
    ...
    return (
        f"https://viewer.nerf.studio/versions/{version}/?websocket_url={websocket_url}"
    )
...
def update_render_aabb(
    ...
    if crop_viewport:
        ...
        if isinstance(model.render_aabb, SceneBox):
            ...
            model.render_aabb = SceneBox(
                aabb=torch.stack([crop_min_tensor, crop_max_tensor], dim=0)
            )
    else:

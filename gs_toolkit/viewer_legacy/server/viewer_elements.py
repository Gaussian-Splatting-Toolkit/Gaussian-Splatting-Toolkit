...
class ViewerControl:
    ...
    def set_pose(
        ...
        assert hasattr(
            self, "viser_server"
        ), "Called set_position on uninitialized ViewerControl"
        self.viser_server.set_camera(
            position=position, look_at=look_at, instant=instant
        )
    ...
    def set_fov(self, fov):
        ...
        assert hasattr(
            self, "viser_server"
        ), "Called set_fov on uninitialized ViewerControl"
    ...
    def set_crop(
        ...
        assert hasattr(
            self, "viser_server"
        ), "Called set_crop on uninitialized ViewerControl"
...
class ViewerButton(ViewerElement[bool]):
    ...
    def __init__(
        self, name: str, cb_hook: Callable[[ViewerButton], Any], disabled: bool = False
    ):
...
class ViewerText(ViewerParameter[str]):
    ...
    def _create_gui_handle(self, viser_server: ViserServer) -> None:
        ...
        self.gui_handle = viser_server.add_gui_text(
            self.name, self.default_value, hint=self.hint
        )
        self.gui_handle.set_disabled(self.disabled)
...
class ViewerVec3(ViewerParameter[Tuple[float, float, float]]):
    ...
    def _create_gui_handle(self, viser_server: ViserServer) -> None:
        self.gui_handle = viser_server.add_gui_vector3(
            self.name, self.default_value, self.step, hint=self.hint
        )
        self.gui_handle.set_disabled(self.disabled)

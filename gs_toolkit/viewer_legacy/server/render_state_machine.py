...
class RenderStateMachine(threading.Thread):
    ...
    def action(self, action: RenderAction):
        ...
        if self.next_action is None:
            ...
            self.state == "low_move"
            or self.next_action.action in ("move", "static", "rerender")
        ):
            # ignore steps if:
    ...
    def _render_img(self, cam_msg: CameraMessage):
        ...
        with self.viewer.train_lock if self.viewer.train_lock is not None else contextlib.nullcontext():
            ...
            isGaussianSplattingModel = isinstance(
                self.viewer.get_model(), GaussianSplattingModel
            )
            if isGaussianSplattingModel:
                # TODO fix me before ship
                camera_ray_bundle = camera.generate_rays(
                    camera_indices=0, aabb_box=self.viewer.get_model().render_aabb
                )
            else:
                camera_ray_bundle = camera.generate_rays(
                    camera_indices=0, aabb_box=self.viewer.get_model().render_aabb
                ...
                if self.viewer.control_panel.crop_viewport:
                    ...
                    if color is None:
                        background_color = torch.tensor(
                            [0.0, 0.0, 0.0], device=self.viewer.pipeline.model.device
                        )
                    else:
                        background_color = torch.tensor(
        ...
        self.viewer.viser_server.send_status_message(
            eval_res=f"{image_height}x{image_width}px", step=step
        )
        return outputs

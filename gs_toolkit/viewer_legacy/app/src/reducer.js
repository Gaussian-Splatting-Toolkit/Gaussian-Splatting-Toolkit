...
const initialState = {
  ...
  file_path_info : {
    config_base_dir :
        'config_base_dir',           // the base directory of the config file
    data_base_dir : 'data_base_dir', // the base directory of the images for
                                     // saving camera path with the data
    export_path_name :
        'export_path_name', // export name for render and camera_path
  },
...
function setData(state, path, data) {
  // If we've reached the final level of the path, update the property with the
  // new data
  if (path.length === 1) {
  ...
  // If we haven't reached the final level of the path, recursively update the
  // nested object by creating a shallow copy of the parent object and updating
...
export default function rootReducer(state = initialState, action) {
  ...
  // If the reducer doesn't recognize the action type, return the existing state
  // unchanged
  default:
    return state;
  }

...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderCUDA(
    ...
    const float *__restrict__ dL_dpixels, const float *__restrict__ dL_depths,
    float3 *__restrict__ dL_dmean2D, float4 *__restrict__ dL_dconic2D,
...
void BACKWARD::render(const dim3 grid, const dim3 block, const uint2 *ranges,
                      ...
                      const float *dL_depths, float3 *dL_dmean2D,
                      float4 *dL_dconic2D, float *dL_dopacity,
                      float *dL_dcolors) {
  renderCUDA<NUM_CHANNELS><<<grid, block>>>(
      ranges, point_list, W, H, bg_color, means2D, conic_opacity, colors,
      depths, final_Ts, n_contrib, dL_dpixels, dL_depths, dL_dmean2D,
      dL_dconic2D, dL_dopacity, dL_dcolors);
}

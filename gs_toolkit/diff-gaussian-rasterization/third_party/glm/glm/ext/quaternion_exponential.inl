...
GLM_FUNC_QUALIFIER qua<T, Q> log(qua<T, Q> const &q) {
  ...
  if (Vec3Len < epsilon<T>()) {
    if (q.w > static_cast<T>(0))
      return qua<T, Q>(log(q.w), static_cast<T>(0), static_cast<T>(0),
                       static_cast<T>(0));
    else if (q.w < static_cast<T>(0))
      return qua<T, Q>(log(-q.w), pi<T>(), static_cast<T>(0),
                       static_cast<T>(0));
    else
      return qua<T, Q>(std::numeric_limits<T>::infinity(),
    ...
    return qua<T, Q>(static_cast<T>(0.5) * log(QuatLen2), t * q.x, t * q.y,
                     t * q.z);
  }
}
...
GLM_FUNC_QUALIFIER qua<T, Q> pow(qua<T, Q> const &x, T y) {
  ...
  if (abs(x.w / magnitude) > cos_one_over_two<T>()) {
    // Scalar component is close to 1; using it to recover angle would lose
    // precision Instead, we use the non-scalar components since sin() is
    // accurate around 0
  ...
  return qua<T, Q>(cos(NewAngle) * magnitude * Mag, x.x * Div * Mag,

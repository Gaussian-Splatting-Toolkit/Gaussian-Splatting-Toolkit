...
from gs_toolkit.cameras.camera_utils import get_interpolated_poses_many
...
def get_interpolated_camera_path(
    ...
    poses, Ks = get_interpolated_poses_many(
        poses, Ks, steps_per_transition=steps, order_poses=order_poses
    )
...
def get_path_from_json(camera_path: Dict[str, Any]) -> Cameras:
    ...
    if all("render_time" in camera for camera in camera_path["camera_path"]):
        times = torch.tensor(
            [camera["render_time"] for camera in camera_path["camera_path"]]
        )
    else:
        times = None

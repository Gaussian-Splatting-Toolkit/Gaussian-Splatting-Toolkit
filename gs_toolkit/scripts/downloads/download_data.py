...
from gs_toolkit.process_data import process_data_utils
from gs_toolkit.scripts.downloads.eyeful_tower import EyefulTowerDownload
from gs_toolkit.scripts.downloads.utils import DatasetDownload
...
class BlenderDownload(DatasetDownload):
    ...
    def download(self, save_dir: Path):
        """Download the blender dataset."""
        # TODO: give this code the same structure as download_gstk
...
@dataclass
class GSTKDownload(DatasetDownload):
    """
    Download data in the GSTK format.
    If you are interested in the GSTK Dataset subset from the SIGGRAPH 2023 paper,
    you can obtain that by using --capture-name GSTK-dataset or by visiting Google Drive directly at:
    https://drive.google.com/drive/folders/19TV6kdVGcmg3cGZ1bNIUnBBMD-iQjRbG?usp=drive_link.
    """

    capture_name: GSTKCaptureName = "bww_entrance"

    def download(self, save_dir: Path):
        """Download the gstk dataset."""
        if self.capture_name == "all":
            for capture_name in gstk_file_ids:
                if capture_name not in ("all", "gstk-dataset"):
                    download_capture_name(save_dir, "gstk", capture_name, gstk_file_ids)
            return

        if self.capture_name == "gstk-dataset":
            for capture_name in gstk_dataset:
                if capture_name not in ("all", "gstk-dataset"):
                    download_capture_name(save_dir, "gstk", capture_name, gstk_file_ids)
            return
...
Commands = Union[
    ...
    Annotated[GSTKDownload, tyro.conf.subcommand(name="gstk")],

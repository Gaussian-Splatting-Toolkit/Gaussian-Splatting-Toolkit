...
meta_dataset = DAVISSaliencyTestDataset(
    args.img_path, args.mask_path, imset=args.imset_path
)
...
for vid_name in pbar:
    ...
    try:
        ...
        vid_reader = meta_dataset.get_offline_sampled_frames(
            vid_name, config["num_voting_frames"]
        )
        ...
        backward_vid_reader = meta_dataset.get_partial_video_loader(
            vid_name, start=-1, end=keyframe_ti + 1, reverse=True
        )
        """
        Forward pass video reader
        """
        forward_vid_reader = meta_dataset.get_partial_video_loader(
            vid_name, start=keyframe_ti, end=-1, reverse=False
        )
        """

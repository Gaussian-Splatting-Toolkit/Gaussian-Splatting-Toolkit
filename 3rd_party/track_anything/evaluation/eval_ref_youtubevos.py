...
from deva.inference.consensus_associated import (
    find_consensus_with_established_association,
)
...
parser.add_argument(
    "--img_path", default="../YouTube/all_frames/valid_all_frames/JPEGImages"
)
parser.add_argument("--mask_path")
parser.add_argument(
    "--json_path", default="../YouTube/meta_expressions/valid/meta_expressions.json"
)
parser.add_argument(
    "--num_voting_frames",
    default=10,
...
meta_dataset = ReferringYouTubeVOSTestDataset(
    args.img_path, args.mask_path, args.json_path
)
...
for vid_name in pbar:
    ...
    for object_name in objects:
        try:
            ...
            vid_reader = meta_dataset.get_offline_sampled_frames(
                vid_name, object_name, config["num_voting_frames"]
            )
            ...
            backward_vid_reader = meta_dataset.get_partial_video_loader(
                vid_name, object_name, start=-1, end=keyframe_ti + 1, reverse=True
            )
            """
            Forward pass video reader
            """
            forward_vid_reader = meta_dataset.get_partial_video_loader(
                vid_name, object_name, start=keyframe_ti, end=-1, reverse=False
            )
            """
            ...
            for vid_reader in vid_readers:
                loader = DataLoader(
                    vid_reader, batch_size=1, shuffle=False, num_workers=2
                )
                ...
                config["enable_long_term_count_usage"] = (
                    ...
                    and (
                        vid_length
                        / (
                            config["max_mid_term_frames"]
                            - config["min_mid_term_frames"]
                        )
                ...
                processor = DEVAInferenceCore(
                    network, config=config, image_feature_store=image_feature_store
                )
                    ...
                    with torch.cuda.amp.autocast(enabled=args.amp):
                        ...
                        if args.save_all or info["save"][0]:
                            this_out_path = path.join(
                                out_path, "Annotations", vid_name, object_name
                            )
                            os.makedirs(this_out_path, exist_ok=True)
                            out_img = Image.fromarray(
                                out_mask.cpu().numpy().astype(np.uint8)
                            )
                            out_img.save(
                                os.path.join(this_out_path, frame[:-4] + ".png")
                            )

            with open(
                path.join(out_path, "Annotations", vid_name, object_name, "key.txt"),
                "w",
            ) as f:

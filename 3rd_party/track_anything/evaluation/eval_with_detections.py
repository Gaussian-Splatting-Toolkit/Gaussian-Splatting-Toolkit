...
from deva.inference.data.vps_test_datasets import (
    VIPSegDetectionTestDataset,
    BURSTDetectionTestDataset,
)
...
parser.add_argument(
    "--dataset", default="vipseg", help="vipseg/burst/unsup_davis17/demo"
)
...
parser.add_argument(
    "--temporal_setting", default="semionline", help="semionline/online"
)
parser.add_argument(
    "--max_num_objects",
    default=-1,
...
if is_vipseg or is_davis or is_demo:
    ...
    meta_dataset = BURSTDetectionTestDataset(
        args.img_path,
        args.mask_path,
        args.burst_gt_json,
        args.size,
        ...
        for ti, data in enumerate(loader):
            with torch.cuda.amp.autocast(enabled=args.amp):
                ...
                if not global_json_enabled:
                    ...
                    segments_info = video_id_to_annotation[vid_name][ti][
                        "segments_info"
                    ]
                    processor.enabled_long_id()
                ...
                segments_info = convert_json_dict_to_objects_info(
                    mask, segments_info, dataset=dataset_name
                )
                    ...
                    if ti + args.num_voting_frames > next_voting_frame:
                        ...
                        if ti == next_voting_frame:
                            ...
                            (
                                ...
                                mask,
                                new_segments_info,
                            ) = processor.vote_in_temporary_buffer(
                        ...
                        prob = processor.step(
                            image, None, None, end=(ti == vid_length - 1)
                        )
                        end.record()
                        torch.cuda.synchronize()
                        total_process_time += start.elapsed_time(end) / 1000

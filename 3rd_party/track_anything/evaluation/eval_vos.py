...
from deva.inference.data.vos_test_datasets import (
    GeneralVOSTestDataset,
    DAVISTestDataset,
...
if is_youtube:
    ...
    meta_dataset = GeneralVOSTestDataset(
        path.join(args.generic_path), size=args.size, use_all_masks=args.use_all_masks
    )
    ...
    try:
        ...
        for ti, data in enumerate(loader):
            with torch.cuda.amp.autocast(enabled=args.amp):
                ...
                prob = processor.step(
                    image, mask, valid_labels, end=(ti == vid_length - 1)
                )
                ...
                if args.save_scores:
                    ...
                    if ti == len(loader) - 1:
                        hkl.dump(
                            processor.object_manager.get_tmp_to_obj_mapping(),
                            path.join(np_path, "backward.hkl"),
                            mode="w",
                        )
                    if args.save_all or info["save"][0]:
                        hkl.dump(

...
class Trainer:
    def __init__(self, config, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        self.DEVA = nn.parallel.DistributedDataParallel(
            DEVA(config).cuda(),
        ...
        self.optimizer = optim.AdamW(
            self.DEVA.parameters(), lr=config["lr"], weight_decay=config["weight_decay"]
        )
    ...
    def do_pass(self, data, it=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.config["amp"]):
            ...
            sensory = torch.zeros(
                (b, num_objects, self.config["value_dim"], *k.shape[-2:])
            )
            v16, sensory = self.DEVA(
                "encode_mask", frames[:, 0], ms_features, sensory, first_frame_gt[:, 0]
            )
            ...
            for ti in range(1, self.num_frames):
                ...
                if ti <= self.num_ref_frames:
                    ...
                    indices = [
                        torch.randperm(ti)[: self.num_ref_frames] for _ in range(b)
                    ]
                    ref_values = torch.stack(
                        [values[bi, :, :, indices[bi]] for bi in range(b)], 0
                    )
                    ref_keys = torch.stack(
                        [keys[bi, :, indices[bi]] for bi in range(b)], 0
                    )
                    ref_shrinkages = (
                        torch.stack(
                            [shrinkages[bi, :, indices[bi]] for bi in range(b)], 0
                        )
                ...
                sensory, logits, masks, aux_logits, aux_masks = self.DEVA(
                    "segment",
                    ms_features,
                    memory_readout,
                    sensory,
        ...
        if self.config["amp"]:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                self.DEVA.parameters(), self.config["clip_grad_norm"]
            )

...
from deva.model.memory_utils import get_similarity, do_softmax
...
def spatial_alignment(
    ...
    sensory = torch.zeros(
        (1, num_objects, config["value_dim"], h // 16, w // 16), device=src_key.device
    )
    value, sensory = network.encode_mask(
        src_image,
        src_ms_features,
        sensory,
        src_mask,
    ...
    memory_readout = memory_readout.view(
        1, num_objects, config["value_dim"], h // 16, w // 16
    )

    # segmentation
    _, _, tar_mask = network.segment(
        tar_ms_features,
        memory_readout,
        sensory,
        src_mask,
...
def find_consensus_with_established_association(
    ...
    if use_score:
        ...
        for ti, image, mask in zip(time_indices, images, masks):
            objective = _keyframe_objective_from_mask(
                mask, score, method="high_foreground"
            )
            if objective > keyframe_objective:
                keyframe_objective = objective
                keyframe_ti = ti

from typing import List, Optional, Dict, Literal, Iterable, Union
import warnings

import torch
from deva.inference.memory_manager import MemoryManager
from deva.inference.object_manager import ObjectManager
from deva.inference.object_utils import ObjectInfo
...
class DEVAInferenceCore:
    ...
    def _add_memory(
        ...
        *,
        ...
        value, sensory = self.network.encode_mask(
            image,
            ms_features,
        ...
        self.memory.add_memory(
            key, shrinkage, value, self.object_manager.all_obj_ids, selection=selection
        )
    ...
    def vote_in_temporary_buffer(
        self, keyframe_selection: Literal["last", "middle", "score", "first"] = "first"
    ) -> Union[int, torch.Tensor, List[ObjectInfo]]:
    ...
    def incorporate_detection(
        ...
        *,
        ...
        (
            ...
            tmp_keep_idx,
            obj_keep_idx,
        ) = self.object_manager.purge_inactive_objects(self.max_missed_detection_count)
    ...
    def step(
        ...
        *,
        ...
        need_segment = (mask is None) or (
            not self.object_manager.has_all(objects) and self.object_manager.num_obj > 0
        )
        ...
        if need_segment:
            pred_prob_with_bg = self._segment(
                key, selection, ms_features, update_sensory=not end
            )
        ...
        if is_mem_frame:
            self._add_memory(
                image, ms_features, self.last_mask, key, shrinkage, selection
            )

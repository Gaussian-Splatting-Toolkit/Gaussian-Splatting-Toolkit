...
def convert_json_dict_to_objects_info(
    mask: torch.Tensor, segments_info: Optional[List], dataset: str = None
) -> List[ObjectInfo]:
    """
    ...
    if segments_info is not None:
        output = [
            ObjectInfo(
                ...
                if dataset in {"burst", "demo"} and "score" in segment
                else None,
            )
        ...
        output = [ObjectInfo(l.item()) for l in labels]  # noqa: E741

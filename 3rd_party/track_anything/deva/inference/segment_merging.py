...
def merge_by_iou(
    our_masks: Dict[ObjectInfo, torch.Tensor],
    new_masks: Dict[ObjectInfo, torch.Tensor],
    our_sums: Dict[ObjectInfo, torch.Tensor],
    new_sums: Dict[ObjectInfo, torch.Tensor],
    merged_mask: torch.Tensor,
    object_manager: ObjectManager,
    new_segments_info: List[ObjectInfo],
    isthing_status: Optional[bool],
...
def match_and_merge(
    ...
    our_masks = {
        obj: (our_mask == tmp) for obj, tmp in object_manager.obj_to_tmp_id.items()
    }
    new_masks = {obj: (new_mask == obj.id) for obj in new_segments_info}

    if (
        max_num_objects > 0
        and len(object_manager.obj_to_tmp_id) + len(new_segments_info) > max_num_objects
    ):

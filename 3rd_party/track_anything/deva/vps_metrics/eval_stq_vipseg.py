...
def eval_stq(submit_dir, truth_dir, pan_gt_json_file):
    ...
    stq_metric = numpy_stq.STQuality(
        n_classes, thing_list_, ignore_label, bit_shift, 2**24
    )
    ...
    for seq_id, video_images in enumerate(pbar):
        ...
        for imgname_j in gt_image_jsons:
            ...
            image = np.array(
                Image.open(os.path.join(submit_dir, "pan_pred", video_id, imgname))
            )
        ...
        for i, (gt_json, pred_json, gt_pan, pred_pan, gt_image_json) in enumerate(
            ...
            stq_metric.update_state(
                ground_truth.astype(dtype=np.int32),
                prediction.astype(dtype=np.int32),
                seq_id,
            )
    ...
    with open(os.path.join(submit_dir, "stq.txt"), "w") as f:
        f.write(
            f'{result["STQ"]*100:.1f},{result["AQ"]*100:.1f},{result["IoU"]*100:.1f}\n'
        )

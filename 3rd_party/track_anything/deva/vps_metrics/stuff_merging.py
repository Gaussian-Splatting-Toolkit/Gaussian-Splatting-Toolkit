...
def process_single_video(vid_ann, input_path, output_path):
    ...
    for ann in vid_ann["annotations"]:
        ...
        output_annotation = {
            "file_name": ann["file_name"],
            "segments_info": output_segments_info,
        }
        ...
        mask = np.array(
            Image.open(
                path.join(
                    input_path, "pan_pred", video_id, file_name.replace(".jpg", ".png")
                )
            )
        ).astype(np.int32)
...
def merge_stuff(input_path, output_path):
    ...
    output_annotations = []
    pool = Pool(16)
    for out_vid_ann in progressbar(
        pool.imap(
            partial(
                process_single_video, input_path=input_path, output_path=output_path
            ),
            annotations,
        ),

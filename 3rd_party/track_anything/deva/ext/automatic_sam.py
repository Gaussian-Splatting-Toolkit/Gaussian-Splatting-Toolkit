...
def auto_segment(
    ...
    if forward_mask is not None:
        ...
        foreground_mask = F.interpolate(
            foreground_mask, scale_factor=1 / 16, mode="bilinear", antialias=True
        )  # blurring
        ...
        points_label = F.grid_sample(
            foreground_mask, points * 2 - 1, align_corners=False
        ).view(-1)
    ...
    if pred_masks.shape[0] == 0:
        ...
        pred_masks = F.interpolate(
            pred_masks.unsqueeze(0), (new_h, new_w), mode="bilinear"
        )[0]
        ...
        if suppress_small_mask:
            ...
            scored_masks_with_bg = torch.cat(
                [
                    torch.zeros((1, *pred_masks.shape[1:]), device=device) + 0.1,
                    scored_masks,
                ],
                dim=0,
            )
            ...
            for k in range(scores.shape[0]):
                ...
                if mask_area > 0 and original_area > 0 and mask.sum() > 0:
                    ...
                    segments_info.append(
                        ObjectInfo(id=curr_id, score=predicted_iou[k].item())
                    )
                    curr_id += 1
        else:
            ...
            scored_masks_with_bg = torch.cat(
                [
                    torch.zeros((1, *scored_masks.shape[1:]), device=device) + 0.1,
                    scored_masks,
                ],
                dim=0,
            )

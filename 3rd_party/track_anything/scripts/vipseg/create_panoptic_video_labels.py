...
if __name__ == "__main__":
    ...
    def conversion_worker(video):
        ...
        for imgname in sorted(os.listdir(os.path.join(original_format_folder, video))):
            original_format = np.array(
                Image.open(os.path.join(original_format_folder, video, imgname))
            )
            ...
            l = np.unique(original_format)  # noqa: E741
            ...
            Image.fromarray(pan_format).save(
                os.path.join(out_folder, video, image_filename)
            )
    ...
    results = pool.map(
        conversion_worker, sorted(os.listdir(original_format_folder)), chunksize=8
    )
